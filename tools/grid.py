...
def process_options(argv=sys.argv):
    ...
    while i < len(argv) - 1:
        if argv[i] == "-log2c":
            ...
            i = i + 1
            ssh_workers = argv[i].split(",")
        elif argv[i] == '-workers':
            i = i + 1
            nr_local_worker = int(argv[i])
        else:
...
def redraw(db, best_param, tofile=False):
    ...
    if all(x[0] == db[0][0] for x in db):
        return
    if all(x[1] == db[0][1] for x in db):
        return
    if all(x[2] == db[0][2] for x in db):
        return

    if tofile:
        gnuplot.write(b"set term png transparent small linewidth 2 \
                      medium enhanced\n")
    ...
    gnuplot.write("set label \"Best log2(C) = {0}  log2(gamma) = {1}  \
                  accuracy = {2}%\" at screen 0.5,0.85 center\n"
...
class SSHWorker(Worker):
    ...
    def run_one(self, c, g):
        ...
        cmdline = cmdstr.format(
            self.host, self.cwd, svmtrain_exe, c, g, fold,
...
class TelnetWorker(Worker):
    def __init__(self, name, job_queue, result_queue, host, username,
                 password):
        Worker.__init__(self, name, job_queue, result_queue)
        self.host = host
...
def main():
    ...
    db = []
    best_rate = -1
    best_c, best_g, best_c1, best_g1 = None, None, None, None
            ...
            while (c, g) not in done_jobs:
                ...
                if (rate > best_rate) or\
                   (rate == best_rate and g1 == best_g1 and c1 < best_c1):
                    ...
                    best_c = 2.0 ** c1
                    best_g = 2.0 ** g1
                print("[{0}] {1} {2} {3} (best c={4}, g={5}, rate={6})".
